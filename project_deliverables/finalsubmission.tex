...
To account for the fact that less than 1\% of the training examples were labeled 1, we first looked only at the 100 tasks where logistic regression performed the best and where at least 1\% of the training examples were labeled 1.
...
\begin{figure*}[!htb]
	\caption{F1 scores for the logistic regression and RNN models for the 20 tasks where logistic regression performed the best. Each selected task had at least 1\% positive training examples. The RNN epoch corresponding to the lowest validation error was epoch 33.}
	\centering
	\includegraphics[scale=0.3]{F1_scores}
\end{figure*}

The RNN's optimal weights were achieved at epoch 33. For the top 100 tasks where logistic regression performed the best (and where at least 1\% of the training examples were labeled 1), the RNN with epoch 33 weights achieved a higher F1 score for 97 of the tasks. On average, the RNN achieved scores that were 1.668 times greater. Taking the max F1 score for each task over all epochs, the RNN beats logistic regression in all 100 tasks, achieving scores that were 2.004 times greater on average. For all 915 tasks with greater than 0 positive examples in the training set, the optimal weights won on 79.9\% of the tasks, and the maximum F1 score over all epochs won on 87.2\% of the tasks. Overall, the RNN outperforms the logistic regression baseline for the classification problem.

With over half of the 919 tasks having less than 1\% positive training examples, we expect the RNN to suffer a significant decrease in performance when these tasks are included. RNN are notoriously dependent on large amounts of training examples, and for these tasks there existed less than 800 training examples. Despite this, the correlation between tasks likely allowed the RNN to leverage examples from other tasks to learn more about a given task. 
